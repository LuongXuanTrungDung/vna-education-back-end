...
export class NguoiDungService {
	...
	async update(id: string, dto: UpdateNguoiDungDto) {
		...
		await this.getOne(id).then(async (doc) => {
			doc.maND = dto.maND
			doc.hoTen = dto.hoTen
			doc.matKhau = await hash(dto.matKhau, 12)
			doc.emailND = dto.emailND
			doc.soDienThoai = dto.soDienThoai
			doc.quocTich = dto.quocTich
			doc.danToc = dto.danToc
			doc.diaChi = dto.diaChi
			doc.gioiTinh = dto.gioiTinh
			doc.ngaySinh = dto.ngaySinh
			doc.ngayNhapHoc = dto.ngayNhapHoc
			doc.cccd = gt
			doc.chucVu = ld
			await doc.save()
		})
